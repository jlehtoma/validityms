...
The spatial resolution of the MS-NFI data is 20x20 meters so all detailed vector data was first converted to rasters of same resolution and extent (SI 1.2). All data preprocessing was done at this resolution, but for the spatial conservation prioritization the data was first aggregated to 60 x 60 meters for computational reasons. We wanted to retain as high resolution as possible because conservation prioritization analyses should be carried out on spatial scale that is in line with with the ecological components (e.g connectivity and average size of habitat patches) and the scale of operative planning [@Arponen2012].
